...
Properly handles the 4 separate screen directories
"""
...
# Load all 4 embedding sets
print("\n[2/6] Loading official embeddings (4 screens)...")

embeddings = {}

# SCREEN_18: Glioblastoma - positive phenotype
...
# SCREEN_18_INVERSE: Glioblastoma - negative phenotype
...
# SCREEN_17: Lung carcinoma - positive phenotype
embeddings['lung_pos'] = {
...
# SCREEN_17_INVERSE: Lung carcinoma - negative phenotype
embeddings['lung_neg'] = {
...
for idx, row in df.iterrows():
    gene = row['gene']
    cell = row['cell']
    method = row['perturbation'].capitalize()  # "knockout" -> "Knockout"
    phenotype = row['phenotype']
    
    # Get appropriate embedding set
    emb_set = get_embedding_set(cell, phenotype)
    
    # Check if all embeddings exist
    if (gene in emb_set['genes'] and 
        cell in emb_set['cells'] and 
        phenotype in emb_set['phenotypes'] and 
        method in emb_set['methods']):
        ...
        # Debug missing
        if gene not in emb_set['genes']:
            print(f"  Missing gene: {gene}")
        if method not in emb_set['methods']:
...
print("="*70)
print("\nUsing organizer-provided embeddings ensures:")
print("  • Standardized evaluation across all teams")
print("  • No embedding generation cost")
